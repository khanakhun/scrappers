...
(async () => {
  ...
  const result = await request.get(
    "https://www.gelbeseiten.de/Suche/fitness/Bundesweit"
  );
  ...
  // })
  const id = $(".mod-Treffer:nth-child(2)").text();
  console.log(id);
  const data = $(
    "#gs_treffer > div > article:nth-child(1) > div:nth-child(1)"
  ).attr("data-realid");
  // console.log(data.attribs);
  $(".mod-TrefferListe").map(async (index, element) => {
